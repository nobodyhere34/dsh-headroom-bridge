...
- **深链跳子智能体压缩记录时「对话/轨迹」反复横跳**：轨迹驱动器的双 tab 兜底会在目标行无法挂载时（子智能体的压缩行在**子会话自己的**轨迹里，父页面永远等不到）每 120ms 交替点中「轨迹」「对话」——现只认轨迹标签（永不点其它 tab）、轨迹选中即停手、表格已挂载而行缺席 8 轮后止损（虚拟折叠的行等多久都不会出现）
