	...
	factory: (require) => {
		...
		*/
		function driveTrajectoryView(callId) {
			...
			const trajectoryTab = (selected) => Array.from(document.querySelectorAll("[role=\"tab\"]")).find((tab) => tab.getAttribute("aria-selected") === "true" === selected && TRAJECTORY_TAB_LABELS.includes((tab.textContent ?? "").trim()));
			const focusRow = () => {
			...
			const step = () => {
				tries++;
				if (trajectoryTab(true) === void 0) trajectoryTab(false)?.click();
				if (focusRow()) return;
