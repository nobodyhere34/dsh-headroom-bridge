...
class ArmAState {
    ...
    get excludeToolReList() { return this.excludeToolRe; }
    get protectPathReList() { return this.protectPathRe; }
    /** Mark-and-check one callId: true on first sight, false afterwards. */
    firstAttempt(callId) {
        if (this.attempts.has(callId))
            return false;
     ...
     */
    async withSlot(fn) {
        if (this.inflight >= this.getConfig().maxInflight)
            return undefined;
        this.inflight++;
        try {
            return await fn();
...
function effectiveToolName(exec) {
 ...
 */
async function attemptCompress(ctx, cfg, state, store, client, counters, toolName, callId, sessionId, model, text) {
    const raw = await state.withSlot(() => client.compressToolMessage({
        toolCallId: callId,
        text,
        model,
 ...
 */
export function installArmA(ctx, getConfig, store, getClient, counters) {
    ...
    const off = ctx.on('tools/post-execute', async (exec, result, next) => {
        ...
        try {
            ...
            if (sourceBlocks === undefined || (Array.isArray(sourceBlocks) && sourceBlocks.length === 0))
                return decision;
            let text = flattenPlainText(sourceBlocks);
            ...
            if (!state.firstAttempt(exec.callId))
                return decision;
            counters.attempts++;
            if (text === undefined)
                return decision; // unreachable past gates; narrows below
            const model = exec.agent?.options.model ?? 'deepseek-chat';
            const outcome = await attemptCompress(ctx, cfg, state, store, getClient(), counters, toolName, exec.callId, String(exec.agent?.id ?? ''), model, text);
            if (typeof outcome === 'string') {
                if (outcome === 'inflight-cap')
                    counters.failures++;
                return decision;
            }
            return {
