 ...
 */
export function renderMarker(hash: string, charsBefore: number, charsAfter: number): string {
...
export function appendMarker(compressed: string, marker: string): string {
  return compressed.endsWith('\n') ? compressed + '\n' + marker : compressed + '\n\n' + marker
}

/** Any headroom CCR marker form: bridge own, row-drop sentinel, prose markers. */
export const ALREADY_COMPRESSED_RE =
  /\[headroom-bridge:|<<ccr:[0-9a-f]{6,}|Retrieve (?:more|original): hash=/i
